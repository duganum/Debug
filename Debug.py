...
st.title("Final Corrected Kinematics: θ from Left & Perpendicular v_B")

# 1. Mechanism Inputs
st.sidebar.header("Mechanism Control")
theta_deg = st.sidebar.slider("Crank Angle (θ) [deg] (Measured from Left)", 0, 180, 60)
...
# θ measured from left means Cartesian angle is 180 - θ
alpha_rad = np.radians(180 - theta_deg)
phi_rad = np.arcsin((r_in * np.sin(np.radians(theta_deg))) / l_in)

# 3. VELOCITY POLYGON
v_b_mag = r_ft * omega_rad
# v_B is 90 degrees to OB (clockwise rotation)
v_b_angle = alpha_rad + np.radians(90)
...
ab_mag = r_ft * (omega_rad**2) # Normal accel B -> O
an_rel_mag = l_ft * (omega_ab**2) # Normal accel A -> B
...
with col2:
    ...
    ax_v.text(vec_va[0], 1, f' vA: {abs(vec_va[0]):.1f}', color='r')
    lim = v_b_mag * 1.5
...
with col3:
    ...
    ax_a.quiver(0, 0, vec_aa[0], 0, color='r', angles='xy', scale_units='xy', scale=1)
    ax_a.text(vec_aa[0], 1000, f' aA: {abs(vec_aa[0]):.0f}', color='r')
    lim = ab_mag * 1.5
