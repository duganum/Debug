...
st.title("Slider-Crank Tutor: Upper-Right Velocity Orientation")
...
# v_B is 90 deg to OB. At theta=60 from left, v_B is at 30 deg (Upper-Right)
v_b_angle = np.radians(90 - (180 - theta_deg)) 
vec_vb = np.array([v_b_mag * np.cos(v_b_angle), v_b_mag * np.sin(v_b_angle)])

# omega_ab calculation for horizontal piston v_A
...
# a_n_rel directed A to B (Up-Right along rod)
...
with col3:
    ...
    ax_a.text(vec_aa[0]/2, 1000, f' aA: {abs(vec_aa[0]):.0f}', color='r')
    lim = ab_mag * 1.5
